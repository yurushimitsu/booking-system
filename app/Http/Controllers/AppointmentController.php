...
class AppointmentController extends Controller
{
    ...
    public function postAppointmentRequest(Request $request) {
        ...
        // $response = Appointment::create($appointmentData);
        ...
        $formalDate = date('F j, Y', strtotime($appointment_date)); // Date for email
        $formalTime = date('g:i A', strtotime($appointment_time));  // Time for email
        $agentName = Agent::select('agent_name')->where('agent_id', $agent_no)->first(); // Agent name for email
        $todayDate = date('F j, Y');
        ...
        try {
            ...
            // Commit the transaction if both are successful
            DB::commit();
            ...
            DB::rollback();
            return response()->json(['success' => false, 'message' => 'Failed to insert data. ' . $e->getMessage()]);
        }
    }
    ...
    public function myBookings() {
        ...
        return view('client.my-bookings', compact('client', 'pendingBookings', 'approvedBookings', 'rejectedBookings', 'bookingCount'));
    }
