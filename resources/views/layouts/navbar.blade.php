      ...
      <ul class="font-medium flex flex-col {{ (Route::is('agents')) ? 'text-gray-900 md:text-white' : 'text-[#06064E]' }} p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-white md:bg-transparent md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:border-0">
        ...
        @if (Session::has('user_role') && session('user_role') == 'client')
          <div class="relative inline-block">
            <!-- Dropdown Button -->
            <button id="dropdownDefaultButton" data-dropdown-toggle="dropdown" class="block py-2 px-3 rounded-sm md:border-0 md:p-0 bg-transparent hover:text-red-500 text-center inline-flex items-center" type="button">
                {{ $client->client_name }}
            ...
            <!-- Dropdown Menu -->
            <div id="dropdown" class="z-10 hidden absolute left-0 mt-2 bg-white divide-y divide-gray-100 rounded-lg shadow-sm md:w-30">
                <ul class="py-2 text-sm text-gray-700" aria-labelledby="dropdownDefaultButton">
                    <li>
                        <a href="{{ route('changePasswordNav') }}" class="block px-4 py-2 hover:bg-gray-100">Account</a>
                    </li>
                    <li>
                        <a href="{{ route('clientLogout') }}" class="block px-4 py-2 hover:bg-gray-100">Logout</a>
                    </li>
                </ul>
            </div>
          </div>
